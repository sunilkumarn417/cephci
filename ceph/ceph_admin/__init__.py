...
class CephAdmin(BootstrapMixin, ShellMixin):
    ...
    def set_cdn_tool_repo(self, release=None):
        ...
        rh_cdn_repos = {
            ...
            "5": {"8": "rhceph-5-tools-for-rhel-8-x86_64-rpms",
                  "9": "rhceph-5-tools-for-rhel-9-x86_64-rpms"},
        }
        ibm_cdn_repos = {
            "7": {"9": "https://public.dhe.ibm.com/ibmdl/export/pub/storage/ceph/ibm-storage-ceph-7-rhel-9.repo"},
            "6": {"9": "https://public.dhe.ibm.com/ibmdl/export/pub/storage/ceph/ibm-storage-ceph-6-rhel-9.repo"},
            "5": {"9": "https://public.dhe.ibm.com/ibmdl/export/pub/storage/ceph/ibm-storage-ceph-5-rhel-9.repo",
        ...
        if ibm_build:
            yum_repo = ibm_cdn_repos[_release][os_major_version]
        else:
            cdn_repo = rh_cdn_repos[_release][os_major_version]

        for node in self.cluster.get_nodes(igonre="client"):
            cmd = f"subscription-manager repos --enable={cdn_repo[os_major_version]}"
            if ibm_build:
                cmd = f"yum-config-manager --add-repo {yum_repo}"
            node.exec_command(sudo=True, cmd=cmd)
